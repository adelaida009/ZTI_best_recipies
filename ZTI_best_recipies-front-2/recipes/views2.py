...
class Test(APIView):
    ...
    def get(self, request):
        array = []
        array.append(
            {'created': "2021-04-30T13:59:04.377000Z",
            'created_by': 1,
            'description': "Tłuste very much",
            'id': 2,
            'ingredients': {'makaron':1, 'pomidor':2},
            'photo': "nie ma jeszcze zdjęcia",
            'slug': "lasagne-bolognese-2021-05-17-1926161217950000",
            'tags': "obiad, makaronowe, spaghetti",
        ...
        array.append(
            {'created': "2021-04-30T13:59:04.377000Z",
             ...
             'tags': "obiad, makaronowe, spaghetti",
             'title': "test2"})
        #return Response({"message": "TEST"}, status=HTTP_200_OK)
        return Response(array, status=HTTP_200_OK)

# class FavouritesView(ListAPIView):
#     def get(self, request):
#         return Response(Favourities.objects.get(user=self.request.query_params.get('user')), status=HTTP_200_OK)

class FavouritesView(ListAPIView): #todo chcialabym zeby mi to dzialalo tak ladnie jak test
    ...
    def get_queryset(self):
        try:
            favourites = Favourities.objects.filter(user=self.request.query_params.get('user'))
            #favourites = Favourities.objects.get(user=self.request.user)
            return favourites
            #return Response({"message": "OKS"}, status=HTTP_200_OK) #todo usunac
        except:
